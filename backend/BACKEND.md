...
- Java 21 / Spring Boot 4.0.1 / JPA (Hibernate) / Spring REST Docs
...
2. `build.gradle`의 `copyDocument` 태스크를 설정하여, 빌드 완료 후 `build/docs/asciidoc`의 결과물을 루트의 `api/` 폴더로 복사하도록 구성합니다.
3. AI는 새로운 API 개발 시, 해당 API에 대응하는 `RestDocs`용 테스트 코드를 반드시 제안해야 합니다.
