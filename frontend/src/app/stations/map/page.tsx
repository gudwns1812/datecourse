...
import { PlaceData, placeService } from "@/services/place";
...
export default function StationMapPage() {
  ...
  const [locationStatus, setLocationStatus] = useState<GeoLocationState>(
    hasValidStation ? "resolved" : "idle"
  );
  const [searchValue, setSearchValue] = useState("");
  const [debouncedSearchValue, setDebouncedSearchValue] = useState("");
  const [selectedCategory, setSelectedCategory] = useState<PlaceCategoryFilter>(
    PLACE_CATEGORY_ALL.value
  );
  ...
  useEffect(() => {
    if (baseMode !== "current" || currentLocation) {
      return;
    }
  ...
  }, [baseMode, currentLocation]);

  const activeLocation = baseMode === "station" ? stationLocation : currentLocation ?? null;
  ...
  useEffect(() => {
    if (!authChecked || !isLoggedIn || activeLatitude === null || activeLongitude === null) {
      return;
    }
    ...
    const loadPlaces = async () => {
      ...
      try {
        const response = await placeService.getPlaces(
          {
            ...
            category:
              selectedCategory === PLACE_CATEGORY_ALL.value ? undefined : selectedCategory,
        ...
        if (response.result === "SUCCESS") {
          ...
          setSelectedPlaceId((previousSelectedId) => {
            if (nextPlaces.some((place) => place.id === previousSelectedId)) {
              ...
              <div className="relative border-b border-slate-200/80 px-6 py-6 md:px-8">
                <div className="flex flex-wrap items-start justify-between gap-4">
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    <Button
                      variant="outline"
                      onClick={() => router.push("/stations/random")}
                      icon="casino"
                    >
                      역 다시 찾기
                    </Button>
                ...
                <div className="mt-6 grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px]">
                  ...
                  <div className="rounded-3xl border border-slate-200 bg-white/80 p-4 dark:border-slate-700 dark:bg-slate-900/50">
                    <div className="flex flex-wrap gap-2">
                      <Badge variant="secondary">추천순</Badge>
                      <Badge variant="secondary">{SEARCH_RADIUS_METERS}m 반경</Badge>
                      <Badge variant="secondary">{SEARCH_RESULT_SIZE}개까지</Badge>
                    </div>
